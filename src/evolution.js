...
(function AI (global, DOMinterface) {
  ...
  function Creature () {
    ...
    this.init = function creatureInit() {
      ...
      this.growthRateSeed = function returnRandomGrowthSeed() { return randomGrowthSeed; };
  ...
  Creature.prototype = {
    ...
    move: function move(target) {
      ...
      if (pursuingATarget) {
        ...
        this.elem.style.height = this.size + 'px';
        this.elem.style.width = this.size + 'px';
        let maxVertical = Math.abs(this.posTop - target.posTop);
        let maxHorizontal = Math.abs(this.posLeft - target.posLeft);
